...
async function main() {
  ...
  if (!args.runId) { process.stderr.write('ocr-post-comments: missing --runId\n'); process.exit(2); }
  ...
  for (const c of comments) {
    ...
    if (c.suggestion_code) body += `\n\n\`\`\`suggestion\n${c.suggestion_code}\n\`\`\``;

    let result: { ok: boolean };
    if (provider === 'github') {
      if (!pr) { process.stderr.write('ocr-post-comments: --pr required for github\n'); process.exit(2); }
      result = await exec('gh', ['pr', 'comment', pr, '--body', body]);
    } else if (provider === 'gitlab') {
      if (!pr) { process.stderr.write('ocr-post-comments: --pr required for gitlab\n'); process.exit(2); }
      result = await exec('curl', ['--request', 'POST',
        `https://gitlab.com/api/v4/projects/${process.env.CI_PROJECT_ID}/merge_requests/${pr}/notes`,
...
main().catch(err => { process.stderr.write(`[ocr-post-comments] ${err.message}\n`); process.exit(1); });
